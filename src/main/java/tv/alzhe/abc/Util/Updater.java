...
public class Updater {
	...
	private static final String HOST = "https://api.curseforge.com"; // Slugs will be appended to
																		// this to get to the
																		// project's RSS feed

	private static final String[] NO_UPDATE_TAG = {"-DEV", "-PRE", "-SNAPSHOT"}; // If the version
																					// number
																					// contains one
																					// of these,
																					// don't update.
	 ...
	 * @param file
	 *            The file that the plugin is running from, get this by doing this.getFile() from
	...
	private void unzip(String file) {
		try {
			...
			// Move any plugin data folders that were included to the right place, Bukkit won't do
			// this for us.
				...
				if (dFile.isDirectory()) {
					if (this.pluginFile(dFile.getName())) {
						final File oFile = new File(this.plugin.getDataFolder().getParent(), dFile.getName()); // Get
																												// current
																												// dir
						final File[] contents = oFile.listFiles(); // List of existing files in the
																	// current dir
						for (final File cFile : dFile.listFiles()) // Loop through all the files in
																	// the new dir
						{
							boolean found = false;
							for (final File xFile : contents) // Loop through contents to see if it
																// exists
							{
